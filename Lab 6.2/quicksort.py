...
def quicksort_range_helper(values, left, right, start, end, style):
    ...
    if left and right > start and end:
        return None
    elif start and end < left and right:
        return None
    else:
...
def pivot_index_mo3(values, left, right):
    ...
    if values[middle] > values[left] and values[middle] > values[right]:#mid high
        if values[left] > values[right]: #right low
            return left
        else:
            return right
 
    elif values[middle] > values[left] and values[middle] < values[right]:#rig high
        return middle 
 

    elif values[middle] < values[left] and values[middle] > values[right]:#lef high
        return middle 
 
    elif values[middle] < values[left] and values[middle] < values[right]:# mid low
        if values[left] < values[right]: #right high
            return left
