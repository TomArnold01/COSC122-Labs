...
class ChainingHashTable():
    ...
    ***** DON'T worry you will fix this later:)
    ...
    def _hash(self, item):
        ...
        hashed_item = nice_hash(item)
        slot_number = hashed_item % self.n_slots
        return slot_number
...
class LinearHashTable():
    ...
    def store(self, item):
        ...
        if self._data[hash_value] is None:
            self._data[hash_value] = item
        else:
            next_slot = hash_value + 1
            if next_slot >= self.n_slots:
                next_slot = 0           
            while self._data[next_slot] != None:
                next_slot = (next_slot + 1)
                if next_slot >= self.n_slots:
                    next_slot = 0 
            self._data[next_slot] = item
        # ===end student section===
...
class QuadraticHashTable():
    ...
    def store(self, item, table_size):
        """Stores an item in the hashtable.
        Notes:
        ...
        # **************************************************
        # ---start student section---
        hash_value = item % table_size #self._hash(item)
        orginal_value = hash_value
        if self._data[hash_value] is None: 
            self._data[hash_value] = item
        else:
            number_of_i = 0
            while self._data[hash_value] is not None:
                number_of_i += 1
                hash_value = orginal_value + (number_of_i**2)
                if hash_value >= self.n_slots:
                    hash_value = hash_value % self.n_slots
    ...
    def __contains__(self, item):
        ...
        # ---start student section---
        if item == self._data[first_hash]:
            return True
        else:
            try_number += 1
            while self._data[current_index] != None:
                current_index = first_hash + try_number**2
                try_number += 1
                if current_index >= self.n_slots:
                    current_index = current_index % self.n_slots
                if item == self._data[current_index]:
                    return True
...
def spellcheck_with_hashtable(document_word_list,
    ...
    table = build_hash_table(ht_type, ht_size, dictionary_word_list, quiet_mode=False)
