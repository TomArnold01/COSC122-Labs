...
def linked_list_print(list_node):
    ...
    if list_node is None:
        ...
        if list_node.next_node != None: # means than there are more items
            print(list_node.item)
            linked_list_print(list_node.next_node)
        else:  # means that this is the last item 
            if list_node.item != None:
                print(list_node.item)
...
def num_rushes(slope_height, rush_height_gain, back_sliding, start_height=0):
    ...
    if rush_height_gain >= slope_height:
        return 1
    elif (total_gain + rush_height_gain ) >= slope_height:
        rushes += 1
        return rushes
    else:
        rushes += 1
        total_gain += (rush_height_gain - back_sliding)
        num_rushes(slope_height, rush_height_gain, back_sliding, start_height + total_gain)
